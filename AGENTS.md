...
- **Prohibido silenciar errores en consola (regla absoluta):** ningún `catch` puede tragarse un error sin registrarlo. Todo error capturado debe registrarse con `console.log`/`console.error` (frontend y backend), tanto si es esperado como inesperado. Prohibido `catch {}` vacío, `catch { /* silencio */ }`, manejadores de eventos (p. ej. `onerror` de SSE/EventSource, `catch` de promises) que no logueen, o suprimir avisos de consola. Si un error se ignora deliberadamente, debe documentarse con un comentario explicando el motivo y registrarse igualmente.
...
- **Prohibido `alert()` en el frontend:** toda notificación al usuario debe mostrarse mediante la librería `vue-greenborn-modal-manager` (paquete npm, ya instalado). Usar `mostrar_modal()` para formularios (con un componente body que reciba `parametros` y se auto-oculte con `ocultar_modal(parametros._modal_cod)`), `mostrar_alerta()` para notificaciones simples y `mostrar_confirm()` para confirmaciones. Prohibido `new Modal()` de Bootstrap directamente o incluir HTML de modales en templates; `<ModalContainer />` ya está montado en `App.vue` y se importa el `style.css` del paquete. Cualquier `alert()` existente debe reemplazarse por el sistema de modales.
- **Prohibido comitear código de repositorios externos:** nunca incluir ni modificar el contenido de directorios que son repositorios externos clonados (p. ej. `opencode_dev/greenborn_1/`, `opencode_dev/`). Su código, dependencias y cambios NO deben commitease ni stagee en este repo; están ignorados en `.gitignore`. No crear o mover archivos de este proyecto dentro de esos directorios externos.
