...
export default {
  setup() {
    ...
    function openVariableDetail(variable) {
      const modalId = modal.open(VariableDetailModal, { variable, proyectoId: proyectoId.value, modalId: null }, { title: variable.key })
      const m = modal.stack.find(m => m.id === modalId)
      if (m) m.props.modalId = modalId
    }
