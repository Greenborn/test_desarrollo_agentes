  ...
  >
    ...
    <g v-for="node in commitNodes" :key="'grp-' + node.hash" class="commit-node">
      ...
      <circle
        :cx="node.cx"
        :cy="node.cy"
        :r="node.isHead ? 5.5 : 4.5"
        :fill="node.color"
        stroke="#0d1117"
        :stroke-width="node.isHead ? 2.5 : 2"
      />
    </g>
    ...
    <g v-for="node in commitNodes" :key="'lb-' + node.hash" class="commit-labels">
      ...
      <g
        ...
        :key="'br-' + b"
        :transform="`translate(${colPositions.msg + 320 + bi * 100}, ${node.cy - 7})`"
      >
        <rect
          width="90"
          height="16"
      ...
      <g
        ...
        :key="'tag-' + t"
        :transform="`translate(${colPositions.msg + 320 + node.branches.length * 100 + ti * 85}, ${node.cy - 7})`"
      >
...
export default {
  ...
  setup(props) {
    ...
    const colPositions = computed(() => {
      const gr = graphRight.value
      const sha = gr + 12
      const author = sha + 88
      const date = author + 178
      const msg = date + 108
    ...
    const commitNodes = computed(() => {
      ...
      for (const n of result) {
        n.color = COLORS[n.lane % COLORS.length]
      }
    ...
    const svgWidth = computed(() => colPositions.value.msg + 500)
