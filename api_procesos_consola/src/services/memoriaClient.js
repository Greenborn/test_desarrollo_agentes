  ...
  return new Promise((resolve, reject) => {
    ...
    ws.on('message', (raw) => {
      ...
      try {
        ...
        console.log('[procesos_memoria] mensaje no JSON recibido:', parseErr.message);
        return;
      }

      if (msg.type === 'auth_result') {
        if (msg.error) {
          console.log('[procesos_memoria] Error de autenticación:', msg.error);
    ...
    ws.on('close', () => {
      console.log('[procesos_memoria] WebSocket desconectado, reintentando en 1s');
      ws = null;
      connecting = false;
    ...
    ws.on('error', (err) => {
      if (err.message || err.code) {
        console.log('[procesos_memoria] Error WebSocket:', err.message || err.code);
      }
    });
  });
}
...
const client = {
  ...
  async health() {
    try {
      ...
      console.log('[procesos_memoria] health check falló:', healthErr.message);
      return false;
    }
  },
};
...
connect().catch((bootErr) => {
  console.log('[procesos_memoria] conexión inicial falló:', bootErr.message);
});
