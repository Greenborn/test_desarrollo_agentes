...
| `/dev_opencode_iniciar` | Inicia una sesión OpenCode: abre el CLI interactivo en una terminal dentro del directorio del proyecto asociado a la sesión. Cada invocación crea una **instancia nueva aislada** (storage propio vía `XDG_DATA_HOME` bajo `~/.local/share/opencode/instances/<id>/`, sembrando `auth.json`), de modo que las instancias no se solapan entre sí. **En la interfaz remota (pseudoendpoint `sendCommand`)** inicia el servidor OpenCode del backend y crea una instancia nueva (requiere `cwd` en la sesión). | `/dev_opencode_iniciar` |
...
> **Nota:** `/ambientes_skills_registrar` agrega paths absolutos a la configuración global (`~/.config/opencode/opencode.json`) para que estén disponibles en **todas** las instancias de OpenCode del sistema, no solo en las del proyecto actual.
